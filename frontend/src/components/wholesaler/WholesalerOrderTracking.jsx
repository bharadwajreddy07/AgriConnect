...
const WholesalerOrderTracking = () => {
    ...
    const loadOrder = async () => {
        try {
            setLoading(true);
            const response = await api.get(`/wholesale-orders/${orderId}`);
        ...
        return (
            <div className="container" style={{ paddingTop: 'var(--spacing-8)' }}>
                <div className="card-premium text-center">
                    ...
                    <Link to="/wholesaler/orders" className="btn btn-primary mt-4">
                        View All Orders
                    </Link>
                </div>
            </div>
        ...
        <div style={{ background: 'linear-gradient(135deg, #f0fdf4 0%, #ecfdf5 100%)', minHeight: '100vh', paddingTop: 'var(--spacing-8)', paddingBottom: 'var(--spacing-12)' }}>
            <div className="container" style={{ maxWidth: '1000px' }}>
                ...
                <div className="flex items-center justify-between mb-6">
                    ...
                    <Link to="/wholesaler/orders" className="btn btn-outline">
                        All Orders
                    </Link>
                </div>
                                                    ...
                                                    {isCompleted && order.statusHistory && (
                                                        <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--gray-600)' }}>
                                                            {order.statusHistory.find(h => h.status === step.key)?.timestamp &&
                                                                formatDate(order.statusHistory.find(h => h.status === step.key).timestamp)}
                                                        </p>
                                                    )}
                                                </div>
                                            </div>
                        ...
                        <div className="card-premium mt-6">
                            ...
                            {showDetails && (
                                <div style={{ marginTop: 'var(--spacing-4)' }}>
                                    <div className="flex gap-4 mb-4" style={{ paddingBottom: 'var(--spacing-4)', borderBottom: '1px solid var(--gray-200)' }}>
                                        <img
                                        ...
                                        />
                                        <div style={{ flex: 1 }}>
                                            <h4 style={{ fontSize: 'var(--font-size-base)' }}>{order.crop?.name}</h4>
                                            <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--gray-600)' }}>
                                                Qty: {order.quantity?.value} {order.quantity?.unit} × {formatPrice(order.pricing?.pricePerUnit)}
                                            </p>
                                            <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--gray-600)' }}>
                                ...
                                <button
                                    onClick={async () => {
                                        if (window.confirm('Are you sure you want to cancel this order?')) {
                                            try {
                                                await api.put(`/wholesale-orders/${order._id}/cancel`, { reason: 'User cancelled' });
...
export default WholesalerOrderTracking;
